...
export class VollPruefModusComponent implements OnInit {
  ...
  maxZahlFehler: number;
  anzahlFragenFehler: number;
  keinWeg: boolean;
  ergAnzeigen: boolean;
  erfolg: boolean;
  letzteFrageErreicht: boolean;
  ...
  ngOnInit(): void {
    ...
    this.maxZahlFehler = 7;
    this.anzahlFragenFehler = 0;
    this.keinWeg = false;

    this.erfolg = false;
    this.letzteFrageErreicht = false;
  }
  ...
  n_frage() {
    ...
    // prüfen auf letzte Frage: wenn ja => Ergebnis anzeigen
    if (!(this.indexFrageNr < this.fragenliste.length - 1)) {
      this.letzteFrageErreicht = true;
    }

    if (
      this.frageNichtBeant() ||
      (this.frageBeant() && this.frageRichtigBeant())
    ) {
      this.keinWeg = false;
      this.toggel = false;
      if (this.indexFrageNr < this.fragenliste.length - 1) {
      ...
      // Fehlermeldung weil Frage falsch beantwortet wurde
      this.anzahlFragenFehler++;
      if (!(this.anzahlFragenFehler == this.maxZahlFehler)) {
        if (this.indexFrageNr < this.fragenliste.length - 1) {
          this.indexFrageNr++;
        }
        this.frage = this.fs.getSingle(this.indexFrageNr);
  ...
  erfolgErgebnis() {
    if (this.anzUnBeFr + this.anzahlFragenFehler < this.maxZahlFehler) {
      this.erfolg = true;
      console.log('Erfolg2', this.erfolg);
    }
  }
